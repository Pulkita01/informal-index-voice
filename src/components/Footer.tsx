...
const Footer = () => {
  ...
  return <footer className="bg-oxford-blue border-t border-glaucous/20">
      <div className="container-width py-20">
        <div className="grid md:grid-cols-4 gap-12">
          ...
          <div className="px-4">
            ...
            <div className="space-y-2">
              {quickLinks.map(link => <button key={link.id} onClick={() => scrollToSection(link.id)} className="block text-glaucous hover:text-eggshell smooth-transition">
                  {link.label}
                </button>)}
            </div>
          </div>
        ...
        <div className="border-t border-glaucous/20 mt-12 pt-8 text-center">
          <p className="text-glaucous text-sm">© 2025 Informal Economy Index. All rights reserved. Built with passion for social impact.</p>
        </div>
      </div>
    </footer>;
};
